...
Declaracao.Visitor<Void> {
	...
	public Void visitVarDeclaracao(Var declaracao) {
		...
		escritor
			.concatenarNaLinha("public static " + tipo + " " + declaracao.nome.lexeme + ";")
			.addQuebraLinha();
		return null;
	}
	...
	public Void visitVarDeclaracoesDeclaracao(VarDeclaracoes declaracao) {
		...
		if (primeiro instanceof Declaracao.Var) {
			...
			escritor
				.concatenarNaLinha("public static " + tipo + " ");

			for (int i = 0; i < lista.size(); i++) {
				Declaracao.Var varriavel = (Declaracao.Var) lista.get(i);
				escritor
					.concatenarNaLinha(varriavel.nome.lexeme);
				if (i < lista.size() - 1) {
					escritor
						.concatenarNaLinha(", ");
				}
			...
			for (int i = 0; i < lista.size(); i++) {
				...
				escritor
					.concatenarNaLinha(varriavel.nome.lexeme + "[" + vv.getTamanho() + "]");
				if (i < lista.size() - 1) {
					escritor
						.concatenarNaLinha(", ");
				}
	...
	public Void visitVariavelArrayDeclaracao(VariavelArray declaracao) {
		...
		escritor
			.concatenarNaLinha("public static " + tipo + " " + declaracao.nome.lexeme + "[" + vv.getTamanho() + "];")
			.addQuebraLinha();
	...
	public Void visitChamadaModuloDeclaracao(ChamadaModulo declaracao) {
		escritor
			.concatenarNaLinha(declaracao.identificador.lexeme + "();")
			.addQuebraLinha();
		return null;
	}
